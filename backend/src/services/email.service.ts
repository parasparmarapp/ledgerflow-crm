...
class EmailService {
  ...
  public initTransporter(): void {
    ...
    const user = process.env.SMTP_USER || process.env.EMAIL_FROM_ADDRESS || '';
    const pass = process.env.SMTP_PASS || process.env.SMTP_SECRET || process.env.EMAIL_PROVIDER_API_KEY || '';
  ...
  public async sendEmail(options: EmailOptions): Promise<EmailSendResult> {
    const fromAddress = process.env.EMAIL_FROM_ADDRESS || process.env.SMTP_USER || 'billing@ledgerflow.local';
    const fromName = (options.fromName || process.env.SMTP_FROM_NAME || 'LedgerFlow CRM Notifications').replace(/[\r\n"]/g, '').trim();
